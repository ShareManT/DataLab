...
$.ajax({
    ...
    success: function (data) {
        if (data.status == "success") {
            ...
            $('#keywordImg').attr('src', `img/KeyWordImage/KeyWordImage${result.keywordImageNumber}.jpg`);
            ...
            // 跳转首页的链接
            const shareTextAddon = `访问你的锦城饭卡消费报告：{{首页链接}}`;
            // 当前用户分享以供他人浏览的链接
            const shareTextPublicAddon = `访问我的锦城饭卡消费报告：{{当前用户的链接}}`;
...
$(function () {
    ...
    $(document).on('click', '.once-watch', function () {
        $.fn.fullpage.moveTo('one');
    })

    // fix 查看详情拖动滚动条页面造成页面拖动
    const htmlHeight = document.documentElement.clientWidth || document.body.clientWidth;
    $('#share-panel-inner').on('touchstart', function () {
        $('#wrap').css('height', htmlHeight);
